...
@injectable()
@registry([{ token: 'Command', useClass: ClearFunctionRunnerExtensionPreferences }])
export class ClearFunctionRunnerExtensionPreferences extends Command {
	...
	async execute(): Promise<void> {
		try {
			...
			const storePromises = Object.keys(StorageKeys).map(key => storageService.store(key as StorageKeys, undefined))
			...
			const logMessage = "Error clearing Function Runner extension's preferences";
