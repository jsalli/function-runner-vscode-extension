...
import { ExtensionMode, OutputChannel, Uri, window } from 'vscode';
...
export class LoggerService {
	...
	private logLevelMode: LogLevel = LogLevel.Off;
	public get logLevel(): LogLevel {
		return this.logLevelMode;
	}
	public set logLevel(value: LogLevel | OutputLevel) {
		this.logLevelMode = fromOutputLevel(value);
		this.orderderLogLevel = toOrderedLevel(this.logLevelMode);
	...
	public logWithDebugParams(message: string, ...params: unknown[]): void {
		if (this.orderderLogLevel < OrderedLevel.Info && !this.isDevelopment) {
			return;
		}
	...
	private toLoggable(
		p: unknown,
		sanitize?: ((key: string, value: unknown) => unknown) | undefined,
	) {
	...
	private toLoggableParams(debugOnly: boolean, params: unknown[]) {
		if (
			params.length === 0 ||
			(debugOnly &&
